...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    if (last_run != 0)
    {
      // If last_run is not 0, calculate the index of the process immediately following last_run
      start_idx = (last_run - ptable.proc + 1) % NPROC;
    }
    else
    {
      // If last_run is 0 (no process has run yet), start from the beginning of the process table
      start_idx = 0;
    }
    ...
    for (int i = 0; i < NPROC; i++)
    {
      ...
      if (highest_priority_proc == 0 || p->nice < lowest_nice)
      {
    ...
    // If a process with the highest priority was found, switch to it.
    if (highest_priority_proc != 0)
    {
      ...
      last_run = highest_priority_proc; // Update the last_run process
      c->proc = 0;
    }
    release(&ptable.lock);
  }
